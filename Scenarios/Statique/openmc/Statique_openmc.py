...
class OpenMCInput:
    ...
    def __init__(self):
        ...
        #Variables
        self._NN = 24 #Number of nodes
        self._batches = 200
        self._inactive = 100
        self._particles = 100000
        self._iteration = 0
    ...
    @property
    def output_dir(self):
        return os.path.join(os.getcwd(), "Results", self.casename, str(self.iteration-1), "TH_output.csv")
    ...
    def main(self):
        ...
        if iteration == 1:
            water_data = {}
            tally_cells = self.geometry_creation(water_data)
        else: 
            water_data = self.TH_extract_data(output_dir)
            tally_cells = self.geometry_creation(water_data)
        mesh = self.mesh_creation()
        self.tallies_creation(iteration, mesh)
        self.settings_creation(mesh)
        self.geometry_plot()
    ...
    def mesh_creation(self):
        ...
        mesh.dimension  = [34, 34, self.NN]
        mesh.lower_left = [self.AG, self.AG, 0.0]
        mesh.upper_right= [self.AG+self.CP*17, self.AG+self.CP*17, self.AH]

        return mesh

    def tallies_creation(self, iteration, mesh):

        results_dir = self.results_dir

        mesh_filter = openmc.MeshFilter(mesh)

        tally = openmc.Tally(name=f"heating_per_cell{iteration}")
        tally.filters = [mesh_filter]
        tally.scores = ["heating"]
        openmc.Tallies([tally]).export_to_xml(results_dir)

    def settings_creation(self, mesh):
    ...
    def geometry_plot(self):
        ...
        if os.path.exists(ppm_file):
            ...
            print("Aucun fichier PPM trouvé")
